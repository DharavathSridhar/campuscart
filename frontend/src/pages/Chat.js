...
const Chat = () => {
  ...
  const isMyMessage = (message) => (typeof message.sender === 'string' ? message.sender : message.sender?._id) === user?._id;
  ...
  useEffect(() => {
    ...
    socket.current.on('receiveMessage', (msg) => {
      ...
      setConversations(prev => prev.map(c => c.userId === senderId ? { ...c, lastMessage: msg } : c));
    });
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const { data } = await API.post('/messages', { receiverId: activeChat, content: newMessage });
      setMessages(prev => [...prev, data.message]);
      setConversations(prev => {
        const updated = prev.map(c => c.userId === activeChat ? { ...c, lastMessage: data.message } : c);
        return updated;
      });
      setNewMessage('');
            ...
            {activeChat ? (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50">
                  {messages.map((msg, i) => (
                    <div key={msg._id || i} className={`flex ${isMyMessage(msg) ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl ${isMyMessage(msg) ? 'bg-primary-500 text-white rounded-br-md' : 'bg-white text-gray-800 shadow-sm rounded-bl-md'}`}>
                        <p className="text-sm">{msg.content}</p>
                        <p className={`text-xs mt-1 ${isMyMessage(msg) ? 'text-primary-100' : 'text-gray-400'}`}>{new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    </div>
                  ))}
